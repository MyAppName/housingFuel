<template>
	<view class="partyBuild">
		<view class="partyBuild_top">
			...
			<view class="Progress">
				<text style="margin-left: 41.66upx;font-size: 26upx !important;">填空题</text>
		...
		<view class="partyBuildContent">
			...
			<view class="selector">
				<view class="selectorItem"  @tap.stop="getTndex(1,$event)"  ref="{{choicechild}}" :class="{'selectorItemBorder':indexColor==1}">
					<view class="circle" :class="{'circleBorder':indexColor==1}"> </view>
					<input type="text" value="" placeholder="请输入答案" @input="inputText"/>
				</view>
...
export default {  
		...
		methods:{
			...
			next:function(){
				console.log(this.choicechild);
				if(this.choicechild!=null&&this.choicechild!=""&&this.choicechild!=undefined){
				this.molecule++;
				let setAnswer={};
				setAnswer.name=this.questionText;
				setAnswer.selector=this.choicechild;
				// 
				if(this.molecule>this.Denominator){
					this.molecule=this.Denominator;
					uni.navigateTo({ url: 'JQuiz', });
					
				}
				uni.request({
				    url: 'https://www.ccweb.xyz/gas/wx/api/setAnswer', //
				    data: {
